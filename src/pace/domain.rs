///! Standardized domain parameters for PACE (ICAO 9303 p11 section 9.5.1)
use std::fmt;

use crate::pace::oids::Mapping;

// The MODP groups below are transcribed from [RFC 5114] sections 2.1 to 2.3.
const MODP_1024_160_P: [u8; 128] = [
    ,
];

const MODP_1024_160_G: [u8; 128] = [
    ,
];

const MODP_1024_160_Q: [u8; 20] = [
    0xF5, 0x18, 0xAA, 0x87, 0x81, 0xA8, 0xDF, 0x27, 0x8A, 0xBA, 0x4E, 0x7D, 0x64, 0xB7, 0xCB, 0x9D,
    0x49, 0x46, 0x23, 0x53,
];

const MODP_2048_224_P: [u8; 256] = [
    ,
];

const MODP_2048_224_G: [u8; 256] = [
    ,
];

const MODP_2048_224_Q: [u8; 28] = [
    0x80, 0x1C, 0x0D, 0x34, 0xC5, 0x8D, 0x93, 0xFE, 0x99, 0x71, 0x77, 0x10, 0x1F, 0x80, 0x53, 0x5A,
    0x47, 0x38, 0xCE, 0xBC, 0xBF, 0x38, 0x9A, 0x99, 0xB3, 0x63, 0x71, 0xEB,
];

const MODP_2048_256_P: [u8; 256] = [
    ,
];

const MODP_2048_256_G: [u8; 256] = [
    ,
];

const MODP_2048_256_Q: [u8; 32] = [
    0x8C, 0xF8, 0x36, 0x42, 0xA7, 0x09, 0xA0, 0x97, 0xB4, 0x47, 0x99, 0x76, 0x40, 0x12, 0x9D, 0xA2,
    0x99, 0xB1, 0xA4, 0x7D, 0x1E, 0xB3, 0x75, 0x0B, 0xA3, 0x08, 0xB0, 0xFE, 0x64, 0xF5, 0xFB, 0xD3,
];

/// A MODP group for DH, as big-endian unsigned integers.
#[derive(Debug)]
pub struct ModpGroup {
    /// Prime p.
    pub p: &'static [u8],
    /// Subgroup generator g.
    pub g: &'static [u8],
    /// Prime order q of g.
    pub q: &'static [u8],
}

pub static MODP_1024_160: ModpGroup = ModpGroup {
    p: &MODP_1024_160_P,
    g: &MODP_1024_160_G,
    q: &MODP_1024_160_Q,
};

pub static MODP_2048_224: ModpGroup = ModpGroup {
    p: &MODP_2048_224_P,
    g: &MODP_2048_224_G,
    q: &MODP_2048_224_Q,
};

pub static MODP_2048_256: ModpGroup = ModpGroup {
    p: &MODP_2048_256_P,
    g: &MODP_2048_256_G,
    q: &MODP_2048_256_Q,
};

// Short Weierstrass parameters for the supported curves, transcribed from
// [RFC 5639] for the Brainpool curves and [RFC 5114] for the NIST ones. The
// Integrated Mapping's point encoding needs raw field arithmetic, which the
// curve crates do not expose.
const BP256R1_P: [u8; 32] = [
    0xA9, 0xFB, 0x57, 0xDB, 0xA1, 0xEE, 0xA9, 0xBC, 0x3E, 0x66, 0x0A, 0x90, 0x9D, 0x83, 0x8D, 0x72,
    0x6E, 0x3B, 0xF6, 0x23, 0xD5, 0x26, 0x20, 0x28, 0x20, 0x13, 0x48, 0x1D, 0x1F, 0x6E, 0x53, 0x77,
];

const BP256R1_A: [u8; 32] = [
    0x7D, 0x5A, 0x09, 0x75, 0xFC, 0x2C, 0x30, 0x57, 0xEE, 0xF6, 0x75, 0x30, 0x41, 0x7A, 0xFF, 0xE7,
    0xFB, 0x80, 0x55, 0xC1, 0x26, 0xDC, 0x5C, 0x6C, 0xE9, 0x4A, 0x4B, 0x44, 0xF3, 0x30, 0xB5, 0xD9,
];

const BP256R1_B: [u8; 32] = [
    0x26, 0xDC, 0x5C, 0x6C, 0xE9, 0x4A, 0x4B, 0x44, 0xF3, 0x30, 0xB5, 0xD9, 0xBB, 0xD7, 0x7C, 0xBF,
    0x95, 0x84, 0x16, 0x29, 0x5C, 0xF7, 0xE1, 0xCE, 0x6B, 0xCC, 0xDC, 0x18, 0xFF, 0x8C, 0x07, 0xB6,
];

const BP384R1_P: [u8; 48] = [
    0x8C, 0xB9, 0x1E, 0x82, 0xA3, 0x38, 0x6D, 0x28, 0x0F, 0x5D, 0x6F, 0x7E, 0x50, 0xE6, 0x41, 0xDF,
    0x15, 0x2F, 0x71, 0x09, 0xED, 0x54, 0x56, 0xB4, 0x12, 0xB1, 0xDA, 0x19, 0x7F, 0xB7, 0x11, 0x23,
    0xAC, 0xD3, 0xA7, 0x29, 0x90, 0x1D, 0x1A, 0x71, 0x87, 0x47, 0x00, 0x13, 0x31, 0x07, 0xEC, 0x53,
];

const BP384R1_A: [u8; 48] = [
    0x7B, 0xC3, 0x82, 0xC6, 0x3D, 0x8C, 0x15, 0x0C, 0x3C, 0x72, 0x08, 0x0A, 0xCE, 0x05, 0xAF, 0xA0,
    0xC2, 0xBE, 0xA2, 0x8E, 0x4F, 0xB2, 0x27, 0x87, 0x13, 0x91, 0x65, 0xEF, 0xBA, 0x91, 0xF9, 0x0F,
    0x8A, 0xA5, 0x81, 0x4A, 0x50, 0x3A, 0xD4, 0xEB, 0x04, 0xA8, 0xC7, 0xDD, 0x22, 0xCE, 0x28, 0x26,
];

const BP384R1_B: [u8; 48] = [
    0x04, 0xA8, 0xC7, 0xDD, 0x22, 0xCE, 0x28, 0x26, 0x8B, 0x39, 0xB5, 0x54, 0x16, 0xF0, 0x44, 0x7C,
    0x2F, 0xB7, 0x7D, 0xE1, 0x07, 0xDC, 0xD2, 0xA6, 0x2E, 0x88, 0x0E, 0xA5, 0x3E, 0xEB, 0x62, 0xD5,
    0x7C, 0xB4, 0x39, 0x02, 0x95, 0xDB, 0xC9, 0x94, 0x3A, 0xB7, 0x86, 0x96, 0xFA, 0x50, 0x4C, 0x11,
];

const NIST_P256_P: [u8; 32] = [
    0xFF, 0xFF, 0xFF, 0xFF, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
];

const NIST_P256_A: [u8; 32] = [
    0xFF, 0xFF, 0xFF, 0xFF, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFC,
];

const NIST_P256_B: [u8; 32] = [
    0x5A, 0xC6, 0x35, 0xD8, 0xAA, 0x3A, 0x93, 0xE7, 0xB3, 0xEB, 0xBD, 0x55, 0x76, 0x98, 0x86, 0xBC,
    0x65, 0x1D, 0x06, 0xB0, 0xCC, 0x53, 0xB0, 0xF6, 0x3B, 0xCE, 0x3C, 0x3E, 0x27, 0xD2, 0x60, 0x4B,
];

const NIST_P384_P: [u8; 48] = [
    0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
    0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFE,
    0xFF, 0xFF, 0xFF, 0xFF, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xFF, 0xFF, 0xFF, 0xFF,
];

const NIST_P384_A: [u8; 48] = [
    0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
    0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFE,
    0xFF, 0xFF, 0xFF, 0xFF, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xFF, 0xFF, 0xFF, 0xFC,
];

const NIST_P384_B: [u8; 48] = [
    0xB3, 0x31, 0x2F, 0xA7, 0xE2, 0x3E, 0xE7, 0xE4, 0x98, 0x8E, 0x05, 0x6B, 0xE3, 0xF8, 0x2D, 0x19,
    0x18, 0x1D, 0x9C, 0x6E, 0xFE, 0x81, 0x41, 0x12, 0x03, 0x14, 0x08, 0x8F, 0x50, 0x13, 0x87, 0x5A,
    0xC6, 0x56, 0x39, 0x8D, 0x8A, 0x2E, 0xD1, 0x9D, 0x2A, 0x85, 0xC8, 0xED, 0xD3, 0xEC, 0x2A, 0xEF,
];

const NIST_P521_P: [u8; 66] = [
    0x01, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
    0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
    0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
    0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
    0xFF, 0xFF,
];

const NIST_P521_A: [u8; 66] = [
    0x01, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
    0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
    0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
    0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
    0xFF, 0xFC,
];

const NIST_P521_B: [u8; 66] = [
    0x00, 0x51, 0x95, 0x3E, 0xB9, 0x61, 0x8E, 0x1C, 0x9A, 0x1F, 0x92, 0x9A, 0x21, 0xA0, 0xB6, 0x85,
    0x40, 0xEE, 0xA2, 0xDA, 0x72, 0x5B, 0x99, 0xB3, 0x15, 0xF3, 0xB8, 0xB4, 0x89, 0x91, 0x8E, 0xF1,
    0x09, 0xE1, 0x56, 0x19, 0x39, 0x51, 0xEC, 0x7E, 0x93, 0x7B, 0x16, 0x52, 0xC0, 0xBD, 0x3B, 0xB1,
    0xBF, 0x07, 0x35, 0x73, 0xDF, 0x88, 0x3D, 0x2C, 0x34, 0xF1, 0xEF, 0x45, 0x1F, 0xD4, 0x6B, 0x50,
    0x3F, 0x00,
];

/// Short Weierstrass parameters of a curve, as big-endian field-width values.
///
/// The curve equation is `y^2 = x^3 + ax + b (mod p)`.
#[derive(Debug)]
pub struct EcParameters {
    pub p: &'static [u8],
    pub a: &'static [u8],
    pub b: &'static [u8],
}

pub static NIST_P256_PARAMETERS: EcParameters = EcParameters {
    p: &NIST_P256_P,
    a: &NIST_P256_A,
    b: &NIST_P256_B,
};

pub static NIST_P384_PARAMETERS: EcParameters = EcParameters {
    p: &NIST_P384_P,
    a: &NIST_P384_A,
    b: &NIST_P384_B,
};

pub static NIST_P521_PARAMETERS: EcParameters = EcParameters {
    p: &NIST_P521_P,
    a: &NIST_P521_A,
    b: &NIST_P521_B,
};

pub static BP256R1_PARAMETERS: EcParameters = EcParameters {
    p: &BP256R1_P,
    a: &BP256R1_A,
    b: &BP256R1_B,
};

pub static BP384R1_PARAMETERS: EcParameters = EcParameters {
    p: &BP384R1_P,
    a: &BP384R1_A,
    b: &BP384R1_B,
};

/// The elliptic curves assigned a standardized domain parameter ID.
///
/// Not all of these are usable by passauf, see [`EcCurve::unsupported_reason`].
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum EcCurve {
    NistP192,
    BrainpoolP192r1,
    NistP224,
    BrainpoolP224r1,
    NistP256,
    BrainpoolP256r1,
    BrainpoolP320r1,
    NistP384,
    BrainpoolP384r1,
    BrainpoolP512r1,
    NistP521,
}

impl EcCurve {
    /// Whether we have an implementation of this curve.
    pub fn is_supported(&self) -> bool {
        return matches!(
            self,
            EcCurve::NistP256
                | EcCurve::BrainpoolP256r1
                | EcCurve::NistP384
                | EcCurve::BrainpoolP384r1
                | EcCurve::NistP521
        );
    }

    /// Why this curve is unusable, for reporting to the user.
    ///
    /// The curves we lack are the ones with no Rust implementation available.
    /// Rolling our own arithmetic for them would be a correctness and
    /// side-channel risk out of proportion to how rarely they appear.
    pub fn unsupported_reason(&self) -> Option<&'static str> {
        return match self {
            curve if curve.is_supported() => None,
            _ => Some("no Rust implementation of this curve is available"),
        };
    }

    /// The curve's short Weierstrass parameters, for the curves we implement.
    pub fn parameters(&self) -> Option<&'static EcParameters> {
        return Some(match self {
            EcCurve::NistP256 => &NIST_P256_PARAMETERS,
            EcCurve::NistP384 => &NIST_P384_PARAMETERS,
            EcCurve::NistP521 => &NIST_P521_PARAMETERS,
            EcCurve::BrainpoolP256r1 => &BP256R1_PARAMETERS,
            EcCurve::BrainpoolP384r1 => &BP384R1_PARAMETERS,
            _ => return None,
        });
    }

    /// ICAO 9303 p11 section 9.5.1 excludes NIST P-224 from the Integrated
    /// Mapping, whose point encoding requires p = 3 mod 4.
    pub fn allows_mapping(&self, mapping: Mapping) -> bool {
        if *self == EcCurve::NistP224 && mapping == Mapping::Integrated {
            return false;
        }
        return true;
    }
}

impl fmt::Display for EcCurve {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        let name = match self {
            EcCurve::NistP192 => "NIST P-192",
            EcCurve::BrainpoolP192r1 => "BrainpoolP192r1",
            EcCurve::NistP224 => "NIST P-224",
            EcCurve::BrainpoolP224r1 => "BrainpoolP224r1",
            EcCurve::NistP256 => "NIST P-256",
            EcCurve::BrainpoolP256r1 => "BrainpoolP256r1",
            EcCurve::BrainpoolP320r1 => "BrainpoolP320r1",
            EcCurve::NistP384 => "NIST P-384",
            EcCurve::BrainpoolP384r1 => "BrainpoolP384r1",
            EcCurve::BrainpoolP512r1 => "BrainpoolP512r1",
            EcCurve::NistP521 => "NIST P-521",
        };
        return write!(f, "{}", name);
    }
}

/// A standardized domain parameter set, selected by parameter ID.
#[derive(Debug)]
pub enum DomainParameter {
    Modp(&'static ModpGroup),
    Ec(EcCurve),
}

impl DomainParameter {
    #[allow(dead_code)]
    pub fn is_supported(&self) -> bool {
        return match self {
            // All three MODP groups are usable.
            DomainParameter::Modp(_) => true,
            DomainParameter::Ec(curve) => curve.is_supported(),
        };
    }

    pub fn unsupported_reason(&self) -> Option<&'static str> {
        return match self {
            DomainParameter::Modp(_) => None,
            DomainParameter::Ec(curve) => curve.unsupported_reason(),
        };
    }
}

/// Look up a standardized domain parameter by its ID.
///
/// Returns None for the IDs the standard reserves for future use (3 to 7 and
/// 19 to 31), which no document should be asking for.
pub fn from_parameter_id(parameter_id: u64) -> Option<DomainParameter> {
    return Some(match parameter_id {
        0 => DomainParameter::Modp(&MODP_1024_160),
        1 => DomainParameter::Modp(&MODP_2048_224),
        2 => DomainParameter::Modp(&MODP_2048_256),
        // 3 to 7 are reserved for future use.
        8 => DomainParameter::Ec(EcCurve::NistP192),
        9 => DomainParameter::Ec(EcCurve::BrainpoolP192r1),
        10 => DomainParameter::Ec(EcCurve::NistP224),
        11 => DomainParameter::Ec(EcCurve::BrainpoolP224r1),
        12 => DomainParameter::Ec(EcCurve::NistP256),
        13 => DomainParameter::Ec(EcCurve::BrainpoolP256r1),
        14 => DomainParameter::Ec(EcCurve::BrainpoolP320r1),
        15 => DomainParameter::Ec(EcCurve::NistP384),
        16 => DomainParameter::Ec(EcCurve::BrainpoolP384r1),
        17 => DomainParameter::Ec(EcCurve::BrainpoolP512r1),
        18 => DomainParameter::Ec(EcCurve::NistP521),
        // 19 to 31 are reserved for future use.
        _ => return None,
    });
}

impl fmt::Display for DomainParameter {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        return match self {
            DomainParameter::Modp(group) => write!(
                f,
                "{}-bit MODP group with {}-bit prime order subgroup",
                group.p.len() * 8,
                group.q.len() * 8
            ),
            DomainParameter::Ec(curve) => write!(f, "{}", curve),
        };
    }
}

#[cfg(test)]
mod tests {
    use super::*;

    /// ICAO 9303 p11 Appendix G.2 quotes the 1024-bit MODP group in full.
    #[test]
    fn modp_1024_matches_worked_example() {
        assert_eq!(MODP_1024_160.p[..4], [0xB1, 0x0B, 0x8F, 0x96]);
        assert_eq!(MODP_1024_160.p[124..], [0x2E, 0x4A, 0x43, 0x71]);
        assert_eq!(MODP_1024_160.g[..4], [0xA4, 0xD1, 0xCB, 0xD5]);
        assert_eq!(MODP_1024_160.g[124..], [0x22, 0xB3, 0xB2, 0xE5]);
        assert_eq!(
            MODP_1024_160.q,
            [
                0xF5, 0x18, 0xAA, 0x87, 0x81, 0xA8, 0xDF, 0x27, 0x8A, 0xBA, 0x4E, 0x7D, 0x64, 0xB7,
                0xCB, 0x9D, 0x49, 0x46, 0x23, 0x53
            ]
        );
    }

    #[test]
    fn modp_group_sizes_match_their_names() {
        assert_eq!((MODP_1024_160.p.len(), MODP_1024_160.q.len()), (128, 20));
        assert_eq!((MODP_2048_224.p.len(), MODP_2048_224.q.len()), (256, 28));
        assert_eq!((MODP_2048_256.p.len(), MODP_2048_256.q.len()), (256, 32));
        // The generator is always the same width as the prime.
        assert_eq!(MODP_2048_224.g.len(), MODP_2048_224.p.len());
        assert_eq!(MODP_2048_256.g.len(), MODP_2048_256.p.len());
    }

    #[test]
    fn reserved_parameter_ids_are_unassigned() {
        for parameter_id in (3..=7).chain(19..=31) {
            assert!(from_parameter_id(parameter_id).is_none());
        }
    }

    #[test]
    fn assigned_parameter_ids_resolve() {
        for parameter_id in (0..=2).chain(8..=18) {
            assert!(from_parameter_id(parameter_id).is_some());
        }
    }

    #[test]
    fn support_matches_available_curves() {
        // BrainpoolP256r1, the one German IDs use.
        assert!(from_parameter_id(13).unwrap().is_supported());
        assert!(from_parameter_id(12).unwrap().is_supported());
        assert!(from_parameter_id(18).unwrap().is_supported());
        // All MODP groups.
        for parameter_id in 0..=2 {
            assert!(from_parameter_id(parameter_id).unwrap().is_supported());
        }
        // The curves with no crate behind them.
        for parameter_id in [8, 9, 10, 11, 14, 17] {
            let parameter = from_parameter_id(parameter_id).unwrap();
            assert!(!parameter.is_supported());
            assert!(parameter.unsupported_reason().is_some());
        }
    }

    #[test]
    fn nist_p224_is_barred_from_integrated_mapping() {
        assert!(!EcCurve::NistP224.allows_mapping(Mapping::Integrated));
        assert!(EcCurve::NistP224.allows_mapping(Mapping::Generic));
        // Every other curve allows both.
        assert!(EcCurve::BrainpoolP256r1.allows_mapping(Mapping::Integrated));
        assert!(EcCurve::NistP256.allows_mapping(Mapping::Integrated));
    }
}
