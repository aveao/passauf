...
- PACE: Password Authenticated Connection Establishment: An access control mechanism for eMRTDs, and the successor to BAC. Where BAC derives its keys straight from the MRZ (so the entropy of the session is the entropy of a document number and two dates, which is low enough to brute-force offline from a recorded exchange), PACE runs a Diffie-Hellman key agreement whose generator is derived from a nonce the chip encrypts under a password-derived key. An eavesdropper therefore gets no offline-guessable material, and a wrong password fails immediately rather than yielding a crackable transcript. The password is either the MRZ or the CAN.
    - The key agreement is ECDH or DH, and the step that turns the nonce into a generator is the "mapping": Generic Mapping does a second, throwaway key exchange, Integrated Mapping maps a pseudo-random field element directly onto the group, and Chip Authentication Mapping extends Generic Mapping to do Chip Authentication at the same time.
    - It is optional to support PACE, but there are some documents that are PACE only, like German IDs and residence permit cards.
- CAN: Card Access Number: A short number printed on the document, usable as the PACE password instead of the MRZ. Handy because it can be read off the front of a card without opening a passport to the identity page.
